...
static void ctime2dos(time_t t, uint16_t *dos_date, uint16_t *dos_time)
{
    ...
    localtime_s(&tm, &t);
        ...
        *dos_time |= tm.tm_hour << 11; /* Bits W11-15: Hour. */
                                    ...
                                    uint32_t comp_size))
{
        ...
        for (i = 0; i < num_memb; i++) {
                ...
                lfh.gp_flag |= 0x1 << 11; /* Bit 11: Language encoding flag (EFS). The filename and comment MUST be encoded using UTF-8*/
...
uint32_t zip_file_size(const char *filename, uint32_t file_size)
{
    size_t name_len;
    uint64_t total;
    uint16_t i;
    ...
    total = CFH_BASE_SZ + name_len;  /* Central File Header */
    total += LFH_BASE_SZ + name_len; /* Local File Header */
    total += file_size;              /* Uncompressed data size. */
...
uint32_t zip_ecd_size(const char *comment)
{
    size_t comment_len;
    uint64_t total;
...
uint32_t zip_write_file_lfh(uint8_t *dst,
    const char *filename,
    ...
    const time_t mtime)
{
    ...
    assert(strlen(filename) <= UINT16_MAX);
    name_len = (uint16_t)strlen(filename);

    data_dst = p + LFH_BASE_SZ + name_len;

    if (hwdeflate(file_data, file_size, data_dst, file_size, &comp_sz) &&
        comp_sz < file_size) {
        lfh.method = ZIP_DEFLATED;
        assert(comp_sz <= UINT32_MAX);
        lfh.comp_size = (uint32_t)comp_sz;
    } else {
        memcpy(data_dst, file_data, file_size);
        lfh.method = ZIP_STORED;
        lfh.comp_size = file_size;
    }

    lfh.extract_ver = (0 << 8) | 20; /* DOS | PKZIP 2.0 */
    lfh.gp_flag = (lfh.method == ZIP_DEFLATED ? (0x1 << 1) : 0x0);
    lfh.gp_flag |= 0x1 << 11; /* Bit 11: Language encoding flag (EFS). The filename and comment MUST be encoded using UTF-8*/
    ctime2dos(mtime, &lfh.mod_date, &lfh.mod_time);
    lfh.crc32 = crc32(file_data, file_size);
    lfh.uncomp_size = file_size;
    lfh.name_len = name_len;
    lfh.extra_len = 0;
    lfh.name = (const uint8_t*)filename;
    p += write_lfh(p, &lfh);
    p += lfh.comp_size;
 
    return (uint32_t)(p - dst); // cd_offset
}

uint32_t zip_write_cfh(uint8_t *dst,
    uint8_t *begin)
{
    struct lfh lfh;
    struct cfh cfh;
    ...
    /* Write the Central Directory based on the Local File Headers. */
    lfh_offset = 0;
    while (begin + lfh_offset < dst) {
        ok = read_lfh(&lfh, begin, SIZE_MAX, lfh_offset);
        assert(ok);
    ...
    assert(begin + lfh_offset == dst);
    return (uint32_t)(p - dst);
}

uint32_t zip_write_ecd(uint8_t *dst,
    uint8_t *begin,
    const char *comment, 
    uint16_t cd_entries,
    uint32_t cd_offset)
{
    ...
    uint32_t eocdr_offset = (uint32_t)(p - begin);

    /* Write the End of Central Directory Record. */
    eocdr.disk_nbr = 0;
    eocdr.cd_start_disk = 0;
    eocdr.disk_cd_entries = cd_entries;// num_memb;
    eocdr.cd_entries = cd_entries;//num_memb;
    eocdr.cd_size = eocdr_offset - cd_offset;
    eocdr.cd_offset = cd_offset;
    eocdr.comment_len = (uint16_t)(comment == NULL ? 0 : strlen(comment));
    eocdr.comment = (const uint8_t*)comment;
